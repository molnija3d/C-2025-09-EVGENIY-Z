...
/*
 * This function skip spaces in provided string and return pointer to the first not space character
 */
...
/*
 * This function skip all characters but not spaces in provided string and return pointer to the first space character
 */
...
/*
 * function to parse a one log linei
 * exaple of log line
 * 18.217.223.118 - admin [19/Jul/2020:07:14:55 +0000] "GET / HTTP/1.1" 301 178 "-" "Mozilla/5.0 (Windows NT 6.1; WOW64; Trident/7.0; rv:11.0) like Gecko" "-"
 *
 */
int parse_log_line(char* line, char** url, long long* bytes, char** referer) {
    char* p = line;
    SKIP_CHARS(p); /* skip ip */
    SKIP_SPACES(p); /* skip spaces */
    SKIP_CHARS(p);   /* skip '-' */
    SKIP_SPACES(p); /* skip spaces */
    SKIP_CHARS(p);  /* skip user_name */
    SKIP_SPACES(p); /* skip spaces */
    ...
    *url = malloc(url_len + 1);
    ...
    /* to use function, or to use macro whats the question!? */
    SKIP_SPACES(p);
    SKIP_CHARS(p); 
    /* SKIP_SPACES(p); */
    p = skip_spaces(p);
    ...
    while (1) {
        ...
        if (file_index < data->total_files) {
            ...
            *(data->next_file_index) = file_index + 1;
            ...
            /* if there is no any files available */
            pthread_mutex_unlock(data->file_mutex);
            break;
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    if (0 < file_count) {
        ...
        fprintf(stderr, "Error during scanning log_dir %s", log_dir);
        return 1;
    }
