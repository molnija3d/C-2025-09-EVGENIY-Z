 ...
 * Возвращает 1, если строка содержит число, иначе 0.
 */
static int is_numeric(const char *str, double *out_val) {
    ...
    // Пропускаем возможные пробелы в конце (strtod останавливается на первом нечисловом символе)
    while (isspace((unsigned char)*endptr))
        endptr++;

    // Если преобразовано хотя бы одно число и достигнут конец строки
