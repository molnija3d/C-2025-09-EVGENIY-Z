...
;;; m proc
m:  ; recursively apply func to every element of the list. Mapping function
    push rbp
    mov rbp, rsp
    sub rsp, 16 ;reserve memory in stack for variables

    test rdi, rdi ;test if pointer to list is not zero
    jz outm

    push rbp
    push rbx

    mov rbx, rdi ;first param - pointer to list
    mov rbp, rsi ;second param - pointer to function()

    mov rdi, [rdi] ;first param - list->value  
    call rsi ;run function()

    mov rdi, [rbx + 8] ;first param - pointer to next (list->next)
    mov rsi, rbp ;pointer to function()
    call m ;recusion, call m again. Apply function to every element of the list
